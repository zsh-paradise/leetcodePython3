...
'''
Created on Mar zsh 2017/11/27
tips: 这个算法就是对所有资源都处理一次，把现在数据库中所有的关联关系算一遍，然后把每个人的资源和算出来的关联关系对比
      如果这个人没有操作资源或者操作的都是偏门的资源就推荐不出来。
      （需要对老师加特征值，对这个资源进行处理）
'''
...
# 加载数据集
def loadDataSet():
    return [[1, 3,3, 4], [2, 3,3, 5], [1, 2, 3, 5], [2, 5]]
...
def getActionIds():
    from time import sleep
    #Installed packages: 'python-votesmart'
    from votesmart import votesmart
    # votesmart.apikey = 'get your api key first'
    votesmart.apikey = 'a7fa40adec6f4a77178799fae4441030'
    actionIdList = []
    billTitleList = []
    fr = open(setting.test_address+'/11.Apriori/recent20bills.txt')
...
def testGenerateRules():
    ...
    # 生成关联规则
    rules = generateRules(L1, supportData1, minConf=0.8)
    print 'rules: ', rules
